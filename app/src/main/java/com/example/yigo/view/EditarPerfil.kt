...
class EditarPerfil : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (userId != null) {
            ...
            Log.e("EditarPerfil", "userId es null")
        }
            ...
            try {
                ...
                withContext(Dispatchers.Main) {
                    if (usuario != null && persona != null) {
                        ...
                        Toast.makeText(this@EditarPerfil, "Error al cargar datos del usuario", Toast.LENGTH_SHORT).show()
                        Log.e("EditarPerfil", "Datos del usuario o persona son null")
                    }
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@EditarPerfil, "Error al cargar los datos: ${e.message}", Toast.LENGTH_SHORT).show()
    ...
    private fun guardarCambios() {
        ...
        userId?.let { id ->
            CoroutineScope(Dispatchers.IO).launch {
                ...
                withContext(Dispatchers.Main) {
                    if (resultado) {
                        Toast.makeText(this@EditarPerfil, "Datos actualizados correctamente", Toast.LENGTH_SHORT).show()
                        finish()
                    } else {
                        Toast.makeText(this@EditarPerfil, "Error al actualizar datos", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
